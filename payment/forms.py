...
class ShippingForm(forms.ModelForm):
	...
	shipping_province = forms.CharField(label="", widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'State'}), required=False)
	...
	class Meta:
		...
		fields = ['shipping_full_name', 'shipping_email', 'shipping_address1', 'shipping_address2', 'shipping_city', 'shipping_province', 'shipping_zipcode', 'shipping_country']
		exclude = ['user',]
...
class PaymentForm(forms.Form):
	...
	card_exp_date = forms.CharField(label="", widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'Expirery date'}), required=True)
